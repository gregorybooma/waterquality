...
$sitename=$_POST['location_name'];
$sitex=$_POST['site_x'];
$sitey=$_POST['site_y'];
$sch_name=$_POST['sch_name'];
$startm=$_POST['startmonth'];
$endm=$_POST['stopmonth'];
$startd=$_POST['startday'];
$endd=$_POST['stopday'];
$starty=$_POST['startyear'];
$endy=$_POST['stopyear'];
$vars=$_POST['varmenu'];

$startdatetime = $starty.'-'.$startm.'-'.$startd.' '.'00:00:00';
$enddatetime = $endy.'-'.$endm.'-'.$endd.' '.'23:59:59';
...
/* not sure why left here -- see if commenting breaks anything and if not then delete
	foreach ($vars as $v) {
...
// the above should work. follow guidance on http://www.sitepoint.com/forums/showthread.php?719060-Multi-Dimensional-Array-Into-csv-File
// to see code for writing rows. don't forget to write header row -- that's covered at top of post
// if that fails, just build full joins per http://postgresql.1045698.n5.nabble.com/GENERAL-full-join-question-td1859595.html

?>
<script type="text/javascript">
    var errorString = "<? if (!empty($errors)) { echo 'Sorry, either you forgot to enter start and stop dates, or at least one variable you selected has no data in the time range you selected. You can remove the ones in question from your selection, or choose a different timeframe. Variables returning null and their data time ranges are listed below. Note that there may be gaps within these periods. \\n\n'; foreach ($errors as $msg) { echo "$msg \\n\\n"; } echo 'Click OK to return to selection.';} ?>";
...
<div id="container" class="container">
		<div class="header">
			<p>
			<a href="http://seagrant.mit.edu/ecosys_health.php"><img src="http://massbay.mit.edu/mitis/header.gif" width="439" height="44" border="0" alt="MIT Sea Grant Center for Coastal Resources"></a>
			</p>
			<p>
...
$vars_units_string = implode(",",$vars);

$plr_query = "select graph_water_vars('$fileprefix','$timepair','$vars_units_string','$sitename');";
...
//	$divheight = count($vars)*250;
	...
	foreach ($vars as $v) {
		if ($var != 'time') {
			...
			echo "<a href= \"$pngpath\" target=\"_blank\"><img src=\"$pngpath\" width=\"600\" height=\"$divheight\" align=\"middle\" style=\"margin-right:10px;\"></img></a>";
		}
	}
...
echo "<p style=\"font-size:.85em;\"><b>WATER QUALITY DATA RECORDED AT $sitename: $startdatetime to $enddatetime</b>";
